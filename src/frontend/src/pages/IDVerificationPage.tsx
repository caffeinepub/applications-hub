...
export const IDVerificationPage: React.FC<IDVerificationPageProps> = ({
  ...
  const validate = (): boolean => {
    ...
    if (!selectedType) {
      ...
      if (!expiry || expiry.length < 5)
        newErrors.expiry = 'ENTER VALID EXPIRY';
      if (!nameOnCard.trim())
        newErrors.nameOnCard = 'ENTER NAME ON CARD';
    } else {
      if (!idNumber.trim())
        newErrors.idNumber = 'ENTER ID NUMBER';
      if (!state.trim())
  ...
  const idTypes: { type: IDType; label: string; icon: string; desc: string; variant: 'blue' | 'gold' | 'teal' }[] = [
    { type: 'debit', label: 'DEBIT CARD', icon: '💳', desc: 'Bank debit card', variant: 'blue' },
    { type: 'credit', label: 'CREDIT CARD', icon: '💳', desc: 'Credit card', variant: 'gold' },
    { type: 'state-id', label: 'STATE ID', icon: '🪪', desc: "Driver's license or state ID", variant: 'teal' },
  ];
  ...
  return (
    <div className="min-h-screen flex flex-col page-enter">
      ...
      <nav style={{
        ...
        <MonitorButton variant="default" size="sm" onClick={() => onNavigate('camera-verify')}>
          ← BACK
        </MonitorButton>
      ...
      <main style={{ flex: 1, padding: '24px 16px', maxWidth: '500px', margin: '0 auto', width: '100%' }}>
          ...
          {idTypes.map(({ type, label, icon, desc, variant }) => (
            ...
            >
              ...
              <span className="pixel-text" style={{
                fontSize: '6px',
                color: selectedType === type ? 'oklch(0.9 0.1 265)' : 'oklch(0.65 0.08 265)',
                lineHeight: '1.8',
                textAlign: 'center',
              }}>
                {label}
        ...
        {(selectedType === 'debit' || selectedType === 'credit') && (
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '12px' }}>
              <div>
                <label htmlFor="card-expiry" style={labelStyle}>EXPIRY</label>
                <input
                  id="card-expiry"
                  type="text"
                  value={expiry}
              ...
              <div>
                <label htmlFor="name-on-card" style={labelStyle}>NAME ON CARD</label>
                <input
                  id="name-on-card"
                  type="text"
                  value={nameOnCard}
        ...
        {selectedType === 'state-id' && (
          <div style={{
            ...
            <div>
              <label htmlFor="id-number" style={labelStyle}>ID NUMBER</label>
              <input
                id="id-number"
                type="text"
                value={idNumber}
            ...
            <div>
              <label htmlFor="id-state" style={labelStyle}>STATE</label>
              <input
                id="id-state"
                type="text"
                value={state}
