...
export const SonicSprite: React.FC<SonicSpriteProps> = ({ size = 'sm', animate = true }) => {
  ...
  useEffect(() => {
    ...
    const moveInterval = setInterval(() => {
      setXPos(prev => {
        ...
        if (next > 90) { setDirection(-1); return prev; }
        if (next < 10) { setDirection(1); return prev; }
        return next;
      });
    }, 50);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Eye pupil */}
        <circle cx="21" cy="9" r="1" fill="black" style={{ opacity: isBlinking ? 0 : 1 }} />

        {/* Eye shine */}
        <circle cx="19.5" cy="7.5" r="0.8" fill="white" style={{ opacity: isBlinking ? 0 : 1 }} />

        {/* Nose */}
        <ellipse cx="23" cy="12" rx="1.2" ry="0.8" fill="oklch(0.3 0.1 340)" />

        {/* Mouth */}
        <path d="M 21 14 Q 23 16 25 14" stroke="oklch(0.3 0.1 340)" strokeWidth="1" fill="none" strokeLinecap="round" />

        {/* Speed lines when running */}
        {animate && frame % 2 === 0 && (
          <>
            <line x1="2" y1="18" x2="-4" y2="18" stroke="oklch(0.7 0.22 245 / 0.6)" strokeWidth="1.5" strokeLinecap="round" />
            <line x1="3" y1="21" x2="-3" y2="21" stroke="oklch(0.7 0.22 245 / 0.4)" strokeWidth="1" strokeLinecap="round" />
            <line x1="4" y1="24" x2="-2" y2="24" stroke="oklch(0.7 0.22 245 / 0.3)" strokeWidth="0.8" strokeLinecap="round" />
          </>
        )}
      </svg>
    </div>
