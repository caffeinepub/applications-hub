...
const sizeClasses = {
  sm: { outer: 'p-[4px]', checker: 'p-[4px]', screen: 'min-h-[44px] px-3 py-2', text: 'text-[7px]' },
  md: { outer: 'p-[5px]', checker: 'p-[5px]', screen: 'min-h-[60px] px-4 py-3', text: 'text-[8px]' },
  lg: { outer: 'p-[6px]', checker: 'p-[6px]', screen: 'min-h-[80px] px-6 py-4', text: 'text-[10px]' },
};
...
export const MonitorButton: React.FC<MonitorButtonProps> = ({
  ...
  const [animState, setAnimState] = useState<'idle' | 'breaking' | 'restoring'>('idle');
  ...
  const frameStyle: React.CSSProperties = {
    ...
    boxShadow: isHovered && animState === 'idle'
    ...
    transform: isHovered && animState === 'idle' ? 'translateY(-2px)' : 'translateY(0)',
    animation: animState === 'breaking'
  ...
  const checkerStyle: React.CSSProperties = {
    ...
    padding: s.checker.includes('4px') ? '4px' : s.checker.includes('5px') ? '5px' : '6px',
    border: `2px solid oklch(0.1 0.02 265)`,
  };

  const screenStyle: React.CSSProperties = {
    background: `linear-gradient(160deg, oklch(0.12 0.04 265) 0%, oklch(0.08 0.02 265) 100%)`,
  ...
  const ballStyle: React.CSSProperties = {
    ...
    height: '28px',
    borderRadius: '50%',
    background: 'radial-gradient(circle at 35% 35%, oklch(0.7 0.2 245), oklch(0.35 0.25 245))',
    ...
    animation: animState === 'breaking' ? 'spindash-zoom 0.5s ease-in forwards' : 'none',
  };
    ...
    >
      <div ref={frameRef} style={frameStyle}>
        <div style={checkerStyle}>
          ...
          >
            {/* Sonic eye */}
            <div style={{ position: 'absolute', width: '8px', height: '8px', borderRadius: '50%', background: 'white', top: '4px', right: '5px' }} />
            <div style={{ position: 'absolute', width: '4px', height: '4px', borderRadius: '50%', background: '#111', top: '5px', right: '7px' }} />
          </div>
          ...
          <div style={screenStyle} className={`${s.screen}`}>
            ...
            <div style={{
              position: 'absolute',
              inset: 0,
              backgroundImage: 'repeating-linear-gradient(0deg, transparent, transparent 3px, oklch(0 0 0 / 0.12) 3px, oklch(0 0 0 / 0.12) 4px)',
              pointerEvents: 'none',
              zIndex: 2,
            }} />
            {/* Screen shine */}
            ...
            {/* Content */}
            <div style={{ position: 'relative', zIndex: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
              {icon && (
                <div style={{ fontSize: size === 'lg' ? '20px' : size === 'md' ? '16px' : '12px', marginBottom: '2px' }}>
                  {icon}
                </div>
              )}
              <div
